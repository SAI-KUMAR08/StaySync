...
const SummaryCard = ({ title, value, icon: Icon, color }) => {
  return (
    <div className={`arch-card p-5 flex items-center gap-4`}>
      ...
      <div>
        <p className="text-[9px] font-bold text-text-secondary uppercase tracking-wider leading-none mb-0.5">
          {title}
        </p>
...
const AdminPayments = () => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/owner/payments${q}`);
  ...
  useEffect(() => {
    ...
    // A tenant's new payment request appears on this list in real time.
    const onPaymentRequestCreated = () => fetchPaymentRequests();
    ...
    return () => {
      ...
      socket.off("payment_request_created", onPaymentRequestCreated);
    };
  ...
  const stats = {
    ...
    pending: payments.filter(
      (p) =>
        ...
        (p.paymentMonth || p.month) === currentMonth &&
        p.year === currentYear
    ).length,
    ...
    <div className="space-y-10 pb-20">
              ...
              <tr key={p._id} className="group">
                <td>
                  <div className="flex items-center gap-3.5">
                    <div
                      className={`w-10 h-10 rounded-xl bg-primary/5 text-primary flex items-center justify-center font-black text-base`}
                    >
                      {p.tenantId?.name?.[0]?.toUpperCase() ||
                        p.tenantId?.personalInfo?.name?.[0]?.toUpperCase() ||
                        "T"}
                    </div>
                    <div>
                      <p className="font-bold text-text-primary">
                        {p.tenantId?.name || p.tenantId?.personalInfo?.name || "Unknown Tenant"}
                      </p>
                      <p className="text-[9px] text-text-secondary font-medium uppercase tracking-wider mt-0.5">
                        Room {p.tenantId?.roomId?.roomNumber || "N/A"}
                      </p>
                    </div>
                  </div>
                </td>
                <td>
                  <div className="space-y-0.5">
                    ...
                    {p.fine > 0 && (
                      <p className="text-[9px] text-[#C62828] font-bold tracking-wider uppercase">
                        incl. ₹{p.fine} fine
                      </p>
                    )}
                  </div>
            ...
            {(!payments || payments.length === 0) && (
              <tr>
                <td colSpan="5" className="px-6 py-28 text-center text-text-secondary/50">
                  ...
                  <p className="font-medium uppercase tracking-wider text-[9px]">
      ...
      <div className="space-y-5 pt-4">
        ...
        {loadingRequests ? (
          ...
          <p className="text-text-secondary/50 text-sm py-4 text-center">
          ...
          <div className="arch-card overflow-hidden">
            {paymentRequests
              ...
              .map((req) => (
                <div
                  key={req._id}
                  className="flex items-center justify-between p-5 border-b border-border/40 last:border-b-0"
                >
                  <div className="flex-1">
                    ...
                    {req.paymentProof && (
                      <a
                        ...
                        className="text-[9px] text-primary font-bold underline"
                      >
                        View Proof
                      </a>
                    )}
        ...
        <div className="modal-overlay">
          <div className="modal-card max-w-sm border border-border/50">
            <div className="p-6 border-b border-border/60 flex justify-between items-center">
              <h4 className="text-lg font-bold font-display text-text-primary tracking-tight uppercase">
                Apply Fine
              </h4>
              <button
            ...
            <form onSubmit={handleAddFine} className="p-6 space-y-6">
              ...
              <p className="text-[9px] text-text-secondary/60 text-center font-medium uppercase tracking-wider">
                Added to base rent of{" "}
                <span className="text-text-primary font-bold">₹{selectedPayment?.amount}</span>.
              </p>
            ...
            <form onSubmit={handleCreatePayment} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddPaymentForm({ ...addPaymentForm, fineAmount: e.target.value })
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    className="field-select"
                    value={addPaymentForm.paymentMonth}
                    onChange={(e) =>
                      setAddPaymentForm({ ...addPaymentForm, paymentMonth: e.target.value })
                    }
                  >
                    {MONTHS.map((m) => (
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddPaymentForm({ ...addPaymentForm, dueDate: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  className="field"
                  placeholder="e.g. Manual entry for June rent"
